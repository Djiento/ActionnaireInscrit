...
@app.route('/admin/delete-investor/<int:investor_id>', methods=['POST'])
@login_required
def delete_investor(investor_id):
    ...
    try:
        ...
        if investor.identity_document:
            file_path = os.path.join(app.config['UPLOAD_FOLDER'], investor.identity_document)
        ...
        flash(f'Investisseur "{investor_name}" supprimé avec succès !', 'success')
    except Exception as e:
        db.session.rollback()
        flash('Erreur lors de la suppression de l\'investisseur.', 'danger')
        app.logger.error(f"Error deleting investor {investor_id}: {e}")
